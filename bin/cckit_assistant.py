...
# Потолок денег на прогон пробы. Пин, а не вкус: при 0.10 прогон обрывается
...
def probe_hooks(home, project, card=None, granted=None):
    ...
    try:
        ...
        # прогон 2026-09-24 при `--max-budget-usd 0.10` оборвался, вернул
        # ПУСТОЙ ответ при стоимости 0.13 — и Stop-хук не сработал вовсе.
        # Проба прочла бы это как «контекст не дошёл» и «хук не сработал»,
        # то есть закричала бы «волк» на исправных хуках. Тот же прогон при
